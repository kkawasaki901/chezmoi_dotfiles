...
M.setup = function(opts)
   ...
   wezterm.on('update-right-status', function(window, _pane)
      ...
      local fmt = valid_opts.date_format
      if fmt:find("%%a") then
         local w = tonumber(wezterm.strftime("%w"))
         local days = { "(日)", "(月)", "(火)", "(水)", "(木)", "(金)", "(土)" }
         local jp_day = days[w + 1]
         -- Safely replace %a with the bracketed Japanese weekday name
         fmt = fmt:gsub("%%a", jp_day)
      end

      cells
         :update_segment_text('date_text', wezterm.strftime(fmt))
